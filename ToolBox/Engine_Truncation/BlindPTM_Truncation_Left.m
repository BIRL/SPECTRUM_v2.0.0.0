...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Candidate_ProteinsList_Modified] = BlindPTM_Truncation_Left(RemainingProteins_Left, PepTol, User_Hop_Threshold,PepUnit, sizeHopeInfo,Hop_Info_name, Hop_Info_AA, Hop_Info_end, Hop_Info_start)
Candidate_ProteinsList_Modified = cell(numel(RemainingProteins_Left),1);
ProteinIndex = 0;
...
if sizeHopeInfo > 0
    %% Outer loop
    for index = 1: numel(RemainingProteins_Left)
        %% Prepare Right Truncation
        protein = RemainingProteins_Left{index};
        ...
        while(loop)
            ...
            diff =  Start - protein.RightIons(ThrI);
            if diff <= User_Hop_Threshold && diff >= -User_Hop_Threshold
                ...
                if AA(1) == Sequence(ThrI+1)
                    ...
                    diff = abs( End - (protein.RightIons(ThrI+1) + M));
                    ...
                    if diff < PepTol
                        for i = ThrI+1 : numel(protein.RightIons)
                            protein.RightIons(i) = protein.RightIons(i) + M;
                        end
                        
                        for i = numel(protein.Sequence)-ThrI : numel(protein.LeftIons)
            ...
            elseif diff > User_Hop_Threshold
                ThrI = ThrI + 1;
                if ThrI >= numel(protein.RightIons)
                    break
                end
                continue;
            elseif diff < -User_Hop_Threshold
        ...
        for HopIndex = 1 : size(Shortlisted_Hops ,1)
            ...
            protein.PTM_seq_idx = [protein.PTM_seq_idx; numel(Sequence)-Shortlisted_Hops{HopIndex,5}];
        end
