...
try
    ...
    if nargin
        ...
        elseif ischar(container)  % container skipped - this is part of the args list...
            varargin = {container, varargin{:}}; %#ok<*CCAT>
    ...
    if isempty(findstr(mfilename,err.message)) %#ok<FSTR>
...
%% Process 'print' option
    function [varargin,listing] = processPrintArgs(varargin) %#ok<VALST>
...
%% Process 'position' option
    function [varargin,foundIdx] = processPositionArgs(varargin) %#ok<VALST>
...
%% Process 'size' option
    function [varargin,foundIdx] = processSizeArgs(varargin) %#ok<VALST>
        if length(varargin)>1
...
%% Process 'class' option
    function [varargin,foundIdx] = processClassArgs(varargin) %#ok<VALST>
        if length(varargin)>1
...
%% Process 'property' option
    function [varargin,foundIdx] = processPropertyArgs(varargin) %#ok<VALST>
        if length(varargin)>1
...
%% Process 'depth' option
    function [varargin,foundIdx] = processDepthArgs(varargin) %#ok<VALST>
        if length(varargin)>1
            level = varargin{2};
            if ~isnumeric(level)
        ...
        try
            ...
            for childIdx = 0 : numChildren-1
                ...
                if childNode.isLeafNode
                    ...
                    visible = visible && isempty(findstr(get(childNode,'Name'),'color="gray"')); %#ok<FSTR>
                    %if any(strcmp(childNode.getName,nodesToUnExpand))
                    %name = char(childNode.getName);
                    if any(cellfun(@(s)~isempty(strmatch(s,char(childNode.getName))),nodesToUnExpand)) || ~visible %#ok<MATCH2>
                            ...
                            for idx = 2 : length(jArray)
                                ...
                                if ~isempty(cbData2)
                                    ...
                                    for cbIdx = 1 : length(cbNames)
                                        ...
                                        if ~isequal(cbData2,cbData) && ~isequal(cbData2{newIdx,2}, cbData{cbIdx,2}) %#ok<FNDSB>
